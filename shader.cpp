...
void SEShader::Init(const char * _filename) {
	...
	if (ifs) {
		...
		HRESULT hr = framework->md3dDevice->CreateVertexShader(
			content,
			length,
			nullptr,
			&vertexShader
	...
	if (ifs) {
		...
		HRESULT hr = framework->md3dDevice->CreatePixelShader(
			content,
			length,
			nullptr,
			&pixelShader
...
void SEShader::Bind() {
	...
	memcpy(mappedSubresource.pData, &VSLocalConstantCopy, sizeof(VSConstantBufferLayout));
